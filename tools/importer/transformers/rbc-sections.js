...
/**
 * Transformer: RBC section breaks + Section Metadata.
 *
 * Inserts an <hr> before each non-first section and a Section Metadata block
 * after each styled section, driven by payload.template.sections from
 * tools/importer/page-templates.json (11 sections for the bank-accounts template).
 *
 * Section selectors are used directly from the template's section.selector arrays
 ...
 * exists — parsers run between hooks and replace matched elements). Metadata is
 * anchored to a marker <hr> in afterTransform. See generate-import-transformer.md
 * "Why both hooks".
 */
